...
def process_chunk(chunk_idx: int, chunk_text: str, extraction_system_prompt: str, extraction_user_prompt_template: str, company_name: str) -> Tuple[List[dict], List[dict]]:
...
def build_and_visualize_knowledge_graph(company_name: str, triples_file: str = None, output_dir: str = None):
    ...
    Provides multiple visualization options and graph analytics.
    """
    import networkx as nx
    from pyvis.network import Network
    import json
    ...
    # 2. Interactive HTML visualization using pyvis
...
def process_chunk_batch(batch_chunks: List[str], start_idx: int, extraction_system_prompt: str, extraction_user_prompt_template: str, company_name: str) -> Tuple[List[dict], List[dict]]:
    """
...
def parallel_process_chunks(chunks: List[str], extraction_system_prompt: str, extraction_user_prompt_template: str, company_name: str, 
...
def parallel_canonicalize_schema(triples: List[dict], relation_definitions: Dict[str, str],
                               ...
                               max_workers: int = 2):  # Reduced number of workers
    """
    ...
    try:
        ...
        embedding_results = {'clustering': {}, 'similarity': {}}
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            def process_embeddings_batch(batch_defs, task_type):
                results = {}
                for definition in batch_defs:
                    try:
                        embedding = get_embedding(definition, task_type)
                        if embedding:
                            results[definition] = embedding
                    except Exception as e:
                        print(f"Error getting embedding for {task_type}: {str(e)}")
                return results
            
            # Submit tasks for both embedding types with smaller batches
            futures = []
            for i in range(0, len(definitions), batch_size):
                batch = definitions[i:i + batch_size]
                futures.append(executor.submit(process_embeddings_batch, batch, "CLUSTERING"))
                futures.append(executor.submit(process_embeddings_batch, batch, "SEMANTIC_SIMILARITY"))
            
            # Collect results
            for future in concurrent.futures.as_completed(futures):
                result = future.result()
                if result:
                    embedding_results['clustering'].update(result)
                    embedding_results['similarity'].update(result)
            ...
            def compute_batch_similarities(rel_batch):
                ...
                for rel1 in rel_batch:
                    for rel2 in relation_keys:
                        if rel1 != rel2:
                            try:
                                cluster_sim = compute_similarity(
                                    embedding_results['clustering'][relation_definitions[rel1]],
                                    embedding_results['clustering'][relation_definitions[rel2]]
                                )
                                semantic_sim = compute_similarity(
                                    embedding_results['similarity'][relation_definitions[rel1]],
                                    embedding_results['similarity'][relation_definitions[rel2]]
                                )
                                batch_results.append((rel1, rel2, cluster_sim, semantic_sim))
                            except Exception as e:
        ...
        for relation in relation_keys:
            ...
            similar_relations = []
            for rel1, rel2, cluster_sim, semantic_sim in similarity_results:
                if rel1 == relation and rel2 not in processed:
                    if cluster_sim > cluster_threshold and semantic_sim > semantic_threshold:
                        similar_relations.append((rel2, (cluster_sim + semantic_sim) / 2))
        ...
        return canonicalize_schema(triples, relation_definitions)
...
def main():
    ...
    if canonical_triples:
        ...
        G = build_and_visualize_knowledge_graph(company_name, "canonical_triples.json", "graph_output")
